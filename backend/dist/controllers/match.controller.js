...
const global_ultis_1 = __importDefault(require("../ultis/global.ultis"));
const tables_ultis_1 = __importDefault(require("../ultis/tables.ultis"));
const api_fixture_1 = require("../data/api-fixture");
const predictions_1 = require("../service/predictions");
const similarity_1 = require("../service/similarity");
const calculationProbality_1 = require("../service/calculationProbality");
const api_topscore_injured_1 = require("../data/api-topscore-injured");
const ia_probability_1 = require("../service/ia_probability");
const getFixture_1 = require("../service/getFixture");
const exceljs_1 = __importDefault(require("exceljs"));
const api_hkjc_1 = require("../data/api-hkjc");
const date_fns_1 = require("date-fns");
const chinese_simplify_1 = require("../service/chinese-simplify");
class MatchController {
    static async getMatchResults() {
        console.log("START....");
        const matchesCol = (0, db_1.collection)(firebase_1.db, tables_ultis_1.default.matches);
        const matchesSnapshot = await (0, db_1.getDocs)(matchesCol);
        ...
        const hkjc = await (0, api_hkjc_1.ApiHKJC)();
        if (hkjc.length == 0) {
            return;
        }
        ...
        let items = [];
        let itemsAdd = [];
        const result = await api_1.API.GET(global_ultis_1.default.footylogicGames);
        if (result.status == 200) {
            const dataLogic = result.data;
            const footylogic = dataLogic.data
                .map((daum) => {
                const filteredEvents = daum.events
                    .filter((event) => ids.includes(event.eventId))
                    .sort((a, b) => new Date(b.kickOff.replace(" ", "T")).getTime() -
            ...
            for (const match of matchesWithInvalidDate) {
                await (0, db_1.deleteDoc)((0, db_1.doc)(firebase_1.db, tables_ultis_1.default.matches, match.eventId));
            }
                ...
                try {
                    ...
                    for (let match of matchesList.filter((x) => x.kickOffDate == daum.label)) {
                        ...
                        if (daum.events.filter(ev => ev.eventId === matchId).length == 0) {
                            await (0, db_1.deleteDoc)((0, db_1.doc)(firebase_1.db, tables_ultis_1.default.matches, matchId));
                        }
                    }
                    ...
                    for (let e in daum.events) {
                        ...
                        const resultDetails = await api_1.API.GET(global_ultis_1.default.footylogicDetails + events.eventId);
                        if (resultDetails.status == 200 && resultDetails.data.statusCode == 200) {
                            const footylogicDetails = resultDetails.data;
                            let item = events;
                            ...
                            if (footylogicDetails.data.awayTeamId && footylogicDetails.data.homeTeamId) {
                                ...
                                item.awayTeamLogo = global_ultis_1.default.footylogicImg + footylogicDetails.data.awayTeamLogo + ".png";
                                item.homeTeamLogo = global_ultis_1.default.footylogicImg + footylogicDetails.data.homeTeamLogo + ".png";
                                item.awayTeamNameEn = footylogicDetails.data.awayTeamName;
                                item.homeTeamNameEn = footylogicDetails.data.homeTeamName;
                                const resultLastGames = await api_1.API.GET(global_ultis_1.default.footylogicRecentForm + "&homeTeamId="
                                ...
                                const [homeZhCN, awayZhCN] = await Promise.all([
                                    (0, chinese_simplify_1.convertToSimplifiedChinese)(homeZh),
                                    (0, chinese_simplify_1.convertToSimplifiedChinese)(awayZh)
                                ]);
                                item.homeLanguages = {
                    ...
                    if (items.filter((i) => daum.label == i.kickOffDate).length != 0) {
                        for (let m in items.filter((i) => daum.label == i.kickOffDate)) {
                            ...
                            const fixture = await (0, getFixture_1.GetFixture)(match);
                            if (fixture) {
                                ...
                                if (fixture.id) {
                                    const predictions = await (0, predictions_1.Predictions)(fixture.id);
                                    if (predictions) {
                                        console.log(1);
                                        ...
                                        if (item.fixture_id && item.league_id && item.homeTeamId && item.awayTeamId) {
                                            playersInjured = await (0, api_topscore_injured_1.ApiTopScoreInjured)(item.fixture_id, item.league_id, item.kickOff.split("-")[0], item.homeTeamId, item.awayTeamId);
                                        }
                                        const resultIa = await (0, ia_probability_1.IaProbality)(item, playersInjured);
                                        if (resultIa) {
                                            ...
                                            const result2 = (0, calculationProbality_1.CalculationProbality)(playersInjured, homeWinRate, awayWinRate, homeForm.split(","), awayForm.split(","));
                                            items.filter((i) => daum.label == i.kickOffDate)[m].ia2 = result2;
                                        }
                                        else {
                                            const result = (0, calculationProbality_1.CalculationProbality)(playersInjured, homeWinRate, awayWinRate, homeForm.split(","), awayForm.split(","));
        ...
        const batch = (0, db_1.writeBatch)(firebase_1.db);
        itemsAdd.forEach(match => {
            const matchRef = (0, db_1.doc)(firebase_1.db, tables_ultis_1.default.matches, match.eventId);
    ...
    static async get2Matchs(req, res) {
        try {
            const matchesCol = (0, db_1.collection)(firebase_1.db, tables_ultis_1.default.matches);
            const matchesSnapshot = await (0, db_1.getDocs)(matchesCol);
            const matchesList = matchesSnapshot.docs.map(doc => {
        ...
        try {
            ...
            if (hkjc.length === 0) {
                ...
                if (dbMatches.length > 0) {
                    const allDbDates = [...new Set(dbMatches.map((m) => {
                            if (!m.kickOff)
                                return null;
                ...
                if (dbMatches.length > 0) {
                    ...
                    const dbDates = [...new Set(dbMatches.map((m) => {
                            if (!m.kickOff)
                                return null;
                            const datePart = m.kickOff.split(' ')[0].split('T')[0];
                            return datePart;
                    ...
                    const futureMatches = dbMatches.filter((match) => {
                        if (!match.kickOff)
                            return false;
                        try {
                            let kickOffTime;
                    ...
                    const futureDates = [...new Set(futureMatches.map((m) => {
                            if (!m.kickOff)
                                return null;
                            const datePart = m.kickOff.split(' ')[0].split('T')[0];
                            return datePart;
                    ...
                    console.log("[getMatchs] Sample database matches:", sortedMatches.slice(0, 2).map((m) => ({ id: m.id || m.eventId, home: m.homeTeamName || 'N/A', away: m.awayTeamName || 'N/A', kickOff: m.kickOff })));
                    if (!res.headersSent) {
            ...
            const existingMatchesCol = (0, db_1.collection)(firebase_1.db, tables_ultis_1.default.matches);
            const existingMatchesSnapshot = await (0, db_1.getDocs)(existingMatchesCol);
            ...
            if (!refresh && dbMatchesArray.length > 0) {
                ...
                if (latestHkjcDate && latestDbDate && latestHkjcDate > latestDbDate) {
                    ...
                    // Subtract 1 hour to be less strict - show matches from today even if they're slightly in the past
                    const cutoffTime = new Date(now.getTime() - 60 * 60 * 1000); // 1 hour ago
                    const futureMatches = dbMatchesArray.filter((match) => {
                        if (!match.kickOff)
                            return false;
                        let kickOffTime;
                        if (match.kickOff.includes('T')) {
                    ...
                    console.log("[getMatchs] Sample matches:", sortedMatches.slice(0, 2).map((m) => ({ id: m.id || m.eventId, home: m.homeTeamName || 'N/A', away: m.awayTeamName || 'N/A', hasPredictions: !!m.predictions, hasIA: !!m.ia })));
                    if (!res.headersSent) {
                        return res.json(sortedMatches);
                    }
                    else {
            ...
            let footylogicEventsMap = new Map();
            try {
                const result = await api_1.API.GET(global_ultis_1.default.footylogicGames);
                if (result.status === 200) {
                    const dataLogic = result.data;
            ...
            for (const hkjcMatch of hkjc) {
                ...
                if (footylogicEvent) {
                    ...
                    if (footylogicEvent.homeTeamLogo) {
                        match.homeTeamLogo = global_ultis_1.default.footylogicImg + footylogicEvent.homeTeamLogo + ".png";
                    }
                    if (footylogicEvent.awayTeamLogo) {
                        match.awayTeamLogo = global_ultis_1.default.footylogicImg + footylogicEvent.awayTeamLogo + ".png";
                    }
                    ...
                    matchDetailsPromises.push(api_1.API.GET(global_ultis_1.default.footylogicDetails + hkjcMatch.id)
            ...
            allMatches.forEach(match => {
                ...
                if (footylogicDetails) {
                    if (footylogicDetails.homeTeamLogo && footylogicDetails.awayTeamLogo) {
                        match.homeTeamLogo = global_ultis_1.default.footylogicImg + footylogicDetails.homeTeamLogo + ".png";
                        match.awayTeamLogo = global_ultis_1.default.footylogicImg + footylogicDetails.awayTeamLogo + ".png";
                    }
            ...
            const calculationPromises = allMatches.map(async (match) => {
                    ...
                    try {
                        ...
                        if (month && day && year) {
                            ...
                            if (team && Array.isArray(team) && team.length > 0) {
                                const fixture = await (0, similarity_1.matchTeamSimilarity)(team, match.homeTeamNameEn, match.awayTeamNameEn);
                ...
                if (needsPredictions && fixture_id) {
                    try {
                        ...
                        const predictions = await (0, predictions_1.Predictions)(fixture_id);
                        if (predictions) {
                    ...
                    try {
                        ...
                        if (fixture_id && match.league_id && match.homeTeamId && match.awayTeamId) {
                            try {
                                ...
                                playersInjured = await (0, api_topscore_injured_1.ApiTopScoreInjured)(fixture_id, match.league_id, dateStr, match.homeTeamId, match.awayTeamId);
                            }
                            catch (error) {
                        ...
                        if (!match.lastGames && match.homeTeamName && match.awayTeamName) {
                            try {
                                const lastGamesResult = await api_1.API.GET(global_ultis_1.default.footylogicDetails + match.eventId);
                        ...
                        const resultIa = await (0, ia_probability_1.IaProbality)(match, playersInjured);
                        if (resultIa) {
                            const total = resultIa.home + resultIa.away;
                            const homeShare = resultIa.home / total;
                            const awayShare = resultIa.away / total;
                            ...
                            const result = (0, calculationProbality_1.CalculationProbality)(playersInjured, homeWinRate, awayWinRate, match.homeForm.split(","), match.awayForm.split(","));
                            if (result) {
                                match.ia = result;
                                matchesWithIA++;
            ...
            const convertPromises = allMatches.map(async (match) => {
                if (match.homeLanguages?.zh && !match.homeLanguages.zhCN) {
                    try {
                        match.homeLanguages.zhCN = await (0, chinese_simplify_1.convertToSimplifiedChinese)(match.homeLanguages.zh) || match.homeLanguages.zh;
                    }
                    catch (error) {
                ...
                if (match.awayLanguages?.zh && !match.awayLanguages.zhCN) {
                    try {
                        match.awayLanguages.zhCN = await (0, chinese_simplify_1.convertToSimplifiedChinese)(match.awayLanguages.zh) || match.awayLanguages.zh;
                    }
                    catch (error) {
            ...
            const savePromises = allMatches.map(async (match) => {
                ...
                await (0, db_1.setDoc)(matchRef, match, { merge: true });
            });
            try {
            ...
            const allMatchesDates = [...new Set(allMatches.map((m) => {
                    if (!m.kickOff)
                        return null;
            ...
            const datesBeforeFilter = [...new Set(allMatches.map((m) => {
                    if (!m.kickOff)
                        return null;
            ...
            const datesAfterFilter = [...new Set(futureMatches.map((m) => {
                    if (!m.kickOff)
                        return null;
                    const datePart = m.kickOff.split(' ')[0].split('T')[0];
                    return datePart;
    ...
    static async getMatchDetails(req, res) {
        ...
        try {
            ...
            if (!refresh) {
                const matchRef = (0, db_1.doc)(firebase_1.db, tables_ultis_1.default.matches, id);
                const matchSnap = await (0, db_1.getDoc)(matchRef);
                if (matchSnap.exists()) {
                    existingMatchData = matchSnap.data();
            ...
            const [resultDetails, gamesResult, hkjcData] = await Promise.all([
                api_1.API.GET(global_ultis_1.default.footylogicDetails + id).catch(err => ({ status: 500, data: null, error: err })),
                api_1.API.GET(global_ultis_1.default.footylogicGames).catch(err => ({ status: 500, data: null, error: err })),
                (0, api_hkjc_1.ApiHKJC)().catch((err) => { console.error("[getMatchDetails] Error fetching HKJC:", err); return []; })
            ]);
            ...
            if (gamesResult.status === 200 && gamesResult.data && gamesResult.data.data) {
                for (const daum of gamesResult.data.data) {
                    if (daum.events && Array.isArray(daum.events)) {
                        const event = daum.events.find((e) => e.eventId === id);
                        if (event) {
                            matchEvent = event;
                            break;
                        }
            ...
            let matchDataFromAPI = {
                ...matchEvent,
                id: id,
                eventId: id,
                homeTeamLogo: footylogicDetails?.homeTeamLogo
                    ? global_ultis_1.default.footylogicImg + footylogicDetails.homeTeamLogo + ".png"
                    : matchEvent.homeTeamLogo,
                awayTeamLogo: footylogicDetails?.awayTeamLogo
                    ? global_ultis_1.default.footylogicImg + footylogicDetails.awayTeamLogo + ".png"
            ...
            const matchData = existingMatchData ? { ...existingMatchData, ...matchDataFromAPI } : matchDataFromAPI;
            ...
            if (!existingMatchData?.homeLanguages?.zhCN || !existingMatchData?.awayLanguages?.zhCN) {
                try {
                    const [homeZhCNResult, awayZhCNResult] = await Promise.all([
                        (0, chinese_simplify_1.convertToSimplifiedChinese)(homeZh).catch(() => homeZh),
                        (0, chinese_simplify_1.convertToSimplifiedChinese)(awayZh).catch(() => awayZh)
                    ]);
            ...
            if (needsFixture) {
                fetchPromises.push((async () => {
                    try {
                        const fixture = await (0, getFixture_1.GetFixture)(matchData);
                        if (fixture && fixture.id) {
            ...
            if (needsLastGames && matchData.homeTeamId && matchData.awayTeamId) {
                fetchPromises.push(api_1.API.GET(global_ultis_1.default.footylogicRecentForm + "&homeTeamId="
            ...
            if (!fixture_id && matchData.kickOffDate) {
                try {
                    ...
                    if (month && day && year) {
                        ...
                        let team = await (0, api_fixture_1.ApiFixtureByDate)(formattedDate);
                        if (team && Array.isArray(team) && team.length > 0) {
                            const fixture = await (0, similarity_1.matchTeamSimilarity)(team, matchData.homeTeamNameEn ?? "", matchData.awayTeamNameEn ?? "");
                            if (fixture && fixture.id) {
            ...
            if (refresh && needsIA && matchData.homeForm && matchData.awayForm) {
                try {
                    ...
                    if (matchData.fixture_id && matchData.league_id && matchData.homeTeamId && matchData.awayTeamId) {
                        try {
                            playersInjured = await (0, api_topscore_injured_1.ApiTopScoreInjured)(matchData.fixture_id, matchData.league_id, matchData.kickOff.split("-")[0], matchData.homeTeamId, matchData.awayTeamId);
                        }
                        catch (error) {
                    ...
                    let homeWinRate = null;
                    let awayWinRate = null;
                    ...
                    else if (matchData.hadHomePct && matchData.hadAwayPct) {
                    ...
                    if (homeWinRate !== null && awayWinRate !== null) {
                        ...
                        if (matchData.lastGames && matchData.homeTeamNameEn && matchData.awayTeamNameEn) {
                            try {
                                const resultIa = await (0, ia_probability_1.IaProbality)(matchData, playersInjured);
                                if (resultIa) {
                                    const total = resultIa.home + resultIa.away;
                                    const homeShare = resultIa.home / total;
                                    const awayShare = resultIa.away / total;
                        ...
                        if (!matchData.ia) {
                            try {
                                const result = (0, calculationProbality_1.CalculationProbality)(playersInjured, homeWinRate, awayWinRate, matchData.homeForm.split(","), matchData.awayForm.split(","));
                                matchData.ia = result;
            ...
            try {
                const matchRef = (0, db_1.doc)(firebase_1.db, tables_ultis_1.default.matches, id);
                await (0, db_1.setDoc)(matchRef, matchData, { merge: true });
    ...
    static async analyzeMatch(req, res) {
        const { id } = req.params;
        try {
            const matchRef = (0, db_1.doc)(firebase_1.db, tables_ultis_1.default.matches, id);
            const matchSnap = await (0, db_1.getDoc)(matchRef);
    ...
    static async excelGenerate(req, res) {
        const { id } = req.params;
        try {
            const matchRef = (0, db_1.doc)(firebase_1.db, tables_ultis_1.default.matches, id);
            const matchSnap = await (0, db_1.getDoc)(matchRef);
            ...
            const fillTeamSection = (title, teamData, winRate, startRow) => {
    ...
    static async excelGenerateAll(req, res) {
        try {
            const matchesCol = (0, db_1.collection)(firebase_1.db, tables_ultis_1.default.matches);
            const matchesSnapshot = await (0, db_1.getDocs)(matchesCol);
            ...
            for (const docSnap of matchesSnapshot.docs) {
                ...
                if (!data.kickOff)
                    continue;
                const date = (0, date_fns_1.format)(new Date(data.kickOff), 'yyyy/MM/dd');
...
function parseToInformationForm(resultRecentForm, home, away) {
    const matchesAway = resultRecentForm.recent8Results.awayTeam;
    const teamFormAway = matchesAway.map(m => m.fullTimeResult).join(",");
    const countResults = (res) => matchesAway.filter(m => m.fullTimeResult === res).length;
    const recentMatch = matchesAway.map(match => ({
        homeTeamName: away,
    ...
    const countResultsHome = (res) => matchesHome.filter(m => m.fullTimeResult === res).length;
    const recentMatchHome = matchesHome.map(match => ({
        homeTeamName: home,
        awayTeamName: match.oppTeamName,
        kickOff: match.kickOff,
        competitionName: match.competitionName,
